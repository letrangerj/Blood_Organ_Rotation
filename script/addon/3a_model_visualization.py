#!/usr/bin/env python3
"""
Phase 3a: Model Visualization
Aging Clock Project - Addon Script

This script creates visualizations for the Phase 3 model results:
...
def load_model_results(result_dir="result"):
    ...
    """
    print("Loading model results from Phase 3...")
    ...
    # Load metrics
    metrics_path = os.path.join(result_dir, "3_cv_metrics.csv")
    metrics_df = pd.read_csv(metrics_path)
    
    # Load coefficients
    coefficients_path = os.path.join(result_dir, "3_model_coefficients.csv")
    coefficients_df = pd.read_csv(coefficients_path)
    
    print(f"Loaded predictions for {len(predictions_df)} samples")
    print(f"Model uses {len(coefficients_df)} CpG sites")
...
Correlation: {metrics['Correlation']:.3f}"""
    ...
    # Save plot
    output_path = os.path.join(figure_dir, "3a_predicted_vs_actual.png")
...
def plot_residuals(results, figure_dir="figure"):
    ...
    ax1.set_title('Residuals vs Actual Age', fontsize=14)
    ...
    ax2.set_title('Residuals vs Predicted Age', fontsize=14)
    ...
    # Save plot
    output_path = os.path.join(figure_dir, "3a_residuals.png")
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_coefficients(results, figure_dir="figure"):
    ...
    Args:
        ...
        figure_dir (str): Directory to save the plot
    """
    print("Creating coefficients plot...")
    ...
    ax.set_title('Aging Clock Model Coefficients\n' + 
                f'{len(coef_df)} CpG Sites Selected by LASSO', fontsize=14)
    ...
    # Save plot
    output_path = os.path.join(figure_dir, "3a_coefficients.png")
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Coefficients plot saved to: {output_path}")
...
def plot_age_acceleration(results, figure_dir="figure"):
    ...
    Args:
        ...
        figure_dir (str): Directory to save the plot
    """
    print("Creating age acceleration plot...")
    ...
    ax.set_title('Distribution of Age Acceleration', fontsize=14)
    ax.legend()
    ax.grid(True, alpha=0.3)
...
Max negative: {df['Age_Acceleration'].min():.2f}"""
    ...
    # Save plot
    output_path = os.path.join(figure_dir, "3a_age_acceleration.png")
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Age acceleration plot saved to: {output_path}")
...
def main():
    ...
    """
    print("Starting Phase 3a: Model Visualization\n")
    ...
    print(f"- Plots saved to 'figure' directory with '3a_' prefix")
